// client/src/components/administrador/ruta3/CrearAula/CrearAula.js
import React, { useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import axiosAdmin from '../../../../axiosConfig/axiosAdministrador'; // ✅ usar admin
...
export default function CrearAula() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ endpoint del ADMIN
      const { data } = await axiosAdmin.post('/api/administrador/crear-aulas', payload, { withCredentials: true });

      setMsg({ type: 'ok', text: 'Aula creada correctamente.' });
      // ✅ navegación del ADMIN
      navigate('/administrador/ruta3/elegir-aula', { replace: true, state: { nuevaAulaId: data?.aula?._id } });
    } catch (err) {
    ...
    <div className="administrador-content">
      ...
      <form className="crearAula-form" onSubmit={onSubmit}>
        ...
        <div className="crearAula-acciones">
          ...
          <button type="button" onClick={() => navigate('/administrador/ruta3')}>Cancelar</button>
        </div>
      </form>
    </div>
  );
