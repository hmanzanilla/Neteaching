// client/src/components/administrador/ruta3/ElegirAula/ElegirAula.js
// client/src/components/administrador/ruta3/ElegirAula/ElegirAula.js

import React, { useEffect, useMemo, useRef, useState } from "react";
import { useNavigate } from "react-router-dom";
import axiosAdministrador from "../../../../axiosConfig/axiosAdministrador"; // ✅ instancia correcta para Admin
...
export default function ElegirAula() {
    ...
    async function fetchData() {
      ...
      try {
        const [resEstado, resAulas] = await Promise.all([
          axiosAdministrador.get("/api/administrador/aulas/estado-actual", { withCredentials: true }),
          axiosAdministrador.get("/api/administrador/crear-aulas", { withCredentials: true }),
        ]);
        if (cancel) return;
  ...
  const refresh = async () => {
    try {
      setLoading(true);
      const res = await axiosAdministrador.get("/api/administrador/aulas/estado-actual", { withCredentials: true });
  ...
  const entrarAula = (aulaId) => {
    navigate(`/administrador/ruta3/aula/${aulaId}`);
  };
  ...
  const eliminarAula = async (id) => {
    ...
    try {
      await axiosAdministrador.delete(`/api/administrador/crear-aulas/${id}`, { withCredentials: true });
  ...
  // ------- Render -------
  return (
    <div className="administrador-content elegirAula-container">
