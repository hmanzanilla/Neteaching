...
const Registro_Principal = () => {
  ...
  // Rutas públicas (muestran barra y footer)
  const RUTAS_PUBLICAS_CON_BARRA = [
  ...
  // Rutas con fondo especial (home y marketing)
  const RUTAS_PUBLICAS_CON_FONDO = [
    '/', '/conocenos', '/contacto', '/privacidad',
    '/productos', '/precios', '/clientes', '/recursos', '/soporte'
  ];

  const isPublicaConFondo = RUTAS_PUBLICAS_CON_FONDO.includes(location.pathname);
  const showBarra = RUTAS_PUBLICAS_CON_BARRA.includes(location.pathname);
  const showHeader = !showBarra;
  const showFooter = showBarra;

  useEffect(() => {
    if (showBarra) window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [location.pathname]); // eslint-disable-line

  return (
    <div className={`registro-container ${isPublicaConFondo ? 'fondo-especial' : ''}`}>
      {location.pathname === '/' && <HomeCarousel />}
      {showBarra && <Barra />}
      {showHeader && <HeaderFallback />}
      ...
      <Routes>
        ...
        {/* Placeholders de subpáginas de Productos (para no romper enlaces del dropdown) */}
...
/* Fallback mínimo de Header (mantiene compatibilidad si no estás
   mostrando el Header real en rutas públicas). Si ya tienes un
   Header completo para privadas, puedes borrar esto. */
const HeaderFallback = () => null;

/* Placeholder visual homogéneo para subpáginas aún no implementadas */
